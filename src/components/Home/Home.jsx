...
const Home = () => {
  ...
  return (
    <div className={`w-full h-full flex flex-col p-[5%] gap-7 overflow-y-auto overflow-x-hidden`}>
          ...
          {RecentProjects.length > 0 ? RecentProjects.map(({ ProjectName, LastEdited }) => {
            ...
            >
              <div className={`w-8/10 h-fit overflow-hidden`}>
                <img className={`w-full object-cover object-center`} src="/public/assets/GetStart2.webp" alt="Img" />
              </div>
            ...
            fontFamily: Weights.ExtraBold,
            fontSize: `${(Sizes.Small.slice(0, -3)) * 1.2}rem`
          }} className={`w-full h-10  flex items-center justify-center`}>No Recent Projects...</div>
          }
        </div>
      </div>
